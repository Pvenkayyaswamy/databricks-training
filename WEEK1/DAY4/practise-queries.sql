...
SELECT customer_id,
       ...
       OVER(PARTITION BY customer_id ORDER BY order_date) AS amount_difference
FROM orders;
...
-- 30. Combine CTEs and window functions to find the top-performing employee
-- in each department based on total sales.
